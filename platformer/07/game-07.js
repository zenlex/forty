...
//basic rectangle for positioning functions
...
Game.World = function (friction = 0.85, gravity = 2) {
  ...
  this.zone_id = "00"; //current zone

  this.carrots = [];
  this.carrot_count = 0;
  this.doors = []; //the array of doors for the level
  this.door = undefined;
...
Game.World.prototype = {
  ...
  setup: function (zone) {
    ...
    for (let index = zone.carrots.length - 1; index >= 0; --index) {
    ...
    /* Generate new doors */
    for (let index = zone.doors.length - 1; index >= 0; --index) {
      let door = zone.doors[index];
      this.doors[index] = new Game.Door(door);
    }

    for (let index = zone.grass.length - 1; index >= 0; --index) {
  ...
  update: function () {
    ...
    for (let index = this.doors.length - 1; index >= 0; --index) {
    ...
    for (let index = this.grass.length - 1; index >= 0; --index) {
